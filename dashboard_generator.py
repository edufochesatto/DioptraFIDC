...
def criar_capa(wb):
    ...
    itens = [
        ...
        (19, "📋 Abas: 0.Capa | 1.Ranking | 2.Top10 | 3.CDI/IMAB | 4.Governança | 5.Propósito | 6.Fontes | 7.Glossário", FONT_N),
    ]
...
def criar_ranking(wb, df):
    ...
    MAPA = {
        ...
        'PCT_SUBORDINADO':'Subordinado (%)', 'OVERCOLLATERALIZATION':'Overcoll. (x)',
    ...
    if 'PL (R$ milhões)' in dx.columns:
        ...
        # Ordena pela primeira coluna numérica
        for c in dx.columns:
    ...
    nc = len(dx.columns)
    if nc == 0:
        print("   [AVISO] DataFrame sem colunas para o ranking.")
        return
    ul = len(dx) + 1
    _cab(ws, 1, nc)
    _zeb(ws, 2, ul, nc)
...
def criar_comparativo(wb, df):
    ...
    if 'RENTABILIDADE' in df.columns:
        for _, row in df.dropna(subset=['RENTABILIDADE']).sort_values(
                'RENTABILIDADE', ascending=False).head(20).iterrows():
    ...
    for c, t in enumerate(cab2, 1): ws.cell(row=linha, column=c, value=t)
    _cab(ws, linha, 6)
    linha += 1
    for ano in sorted(BENCH["CDI"]):
        cdi = BENCH["CDI"][ano]; imab = BENCH["IMAB"][ano]
        ipca = BENCH["IPCA"][ano]
...
def criar_governanca(wb, metricas):
    ...
    MAPA = {
        'VL_PL':'PL (R$ milhões)', 'PRAZO_MEDIO':'Prazo Médio (dias)',
        'PDD_PCT':'PDD (% PL)', 'RECOMPRA_PCT':'Recompra (%)',
        'CONC_TOP5_SACADOS':'Conc. Top5 Sacados (%)',
        'CONC_TOP5_CEDENTES':'Conc. Top5 Cedentes (%)',
        'NUM_SACADOS':'Nº Sacados', 'NUM_CEDENTES':'Nº Cedentes',
        'OVERCOLLATERALIZATION':'Overcollateralization (x)',
        'PCT_LIQUIDEZ':'Liquidez (%)', 'PCT_VENCIDOS':'Vencidos +90d (%)',
    ...
    INTERP = {
        ...
        'OVERCOLLATERALIZATION':'🛡️ >1.2x: saudável. <1.0x: crítico.',
